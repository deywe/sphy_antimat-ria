...
window.fullscreen = False  # Mude para True se quiser que ocupe a tela toda sem bordas
window.size = (1920, 1080) # Força a resolução 1080p
...
# HUD de Auditoria - Reposicionado para 16:9
...
def update():
    ...
    # Atualiza a geometria em tempo real
    mesh_entity.model.vertices = [Vec3(xs[i], zs[i], ys[i]) for i in range(len(zs))]
    
    # Coloração por Tensão de Fase (HSV)
    cor_materia = color.color(280, 1, 1)  # Roxo
    cor_radiacao = color.color(60, 1, 1)   # Amarelo
